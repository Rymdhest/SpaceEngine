...
namespace SpaceEngine.Entity_Component_System.Components
{
    class PointLight : Component
    {
        ...
        private Model lightVolumeModel;
        public PointLight(Vector3 color, Vector3 attenuation)
        {
            ...
            radius = (-linear + (float)Math.Sqrt(linear * linear - 4 * quadratic * (constant - (256.0f / 5.0f) * lightMax))) / (2f * quadratic);
            lightVolumeModel = glLoader.loadToVAO(MeshGenerator.generateIcosahedron(radius));
        }
    }
}
